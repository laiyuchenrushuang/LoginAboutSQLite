...
public class UserDataManager {
    ...
    //更新用户信息，如修改密码
    public void updateUserData(String userName,String userPwd) {
        ContentValues values = new ContentValues();
        values.put(USER_NAME, userName);
        values.put(USER_PWD, userPwd);
        Log.i(TAG, "updateUserData USER_NAME ="+userName +" USER_PWD= "+userPwd);
        mSQLiteDatabase.update(TABLE_NAME, values,null, null);
    }
    ...
    public Boolean findUserByName(String userName){
        ...
        if (mCursor.moveToFirst()) {
            for (int i = 0;i<mCursor.getCount();i++) {
                Log.i(TAG,"mCursor.getCount()="+mCursor.getCount() +"  i="+i);
